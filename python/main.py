...
def routine(details: AlgorithmDetails, x, freq: int = None) -> Tuple[str, int, np.ndarray[float]]:
    waveform = NAIVE_SAW
    if freq is None:
        name = details.name
    else:
...
def plot_specgram(time_signals: Dict[str, np.ndarray[float]]):
    ...
    for i, (name, data) in enumerate(time_signals.items()):
        axs[i].specgram(data, NFFT=512, noverlap=256)
...
if __name__ == "__main__":
    ...
    if args.mode == "sweep":
        for options in ALGOS_OPTIONS:
            x = generate_sweep_phase(1, SAMPLERATE / 2, DURATION_S, SAMPLERATE * options.oversampling)
            routine_args.append([options, x])
    else:
        for freq in FREQS:
    ...
    if args.export in ("thd", "both"):
        ...
        for [name, freq, data, snr_value] in results:
            sorted_thd[freq].append(compute_thd(data, sorted_bl[freq]))
